...
def main(argc, argv,dictData) :
    ...
    print(saveRootDirPath + "/" + title + ".mp4")
    # 9.合并ts
...
if __name__ == "__main__" :
    ...
    # # 获取标题
    req_obj = requests.get(_homeurl,_header)
    req_obj.encoding = req_obj.apparent_encoding
    soup = BeautifulSoup(req_obj.text, 'html.parser')
    html = soup.find_all('a', class_='bl')
    ...
    for href in html :
        ...
        if title is not None:
            href = 'https://www.yiihuu.com'+href.get('href')
            title_list.append(title)
            url_list.append(href)
            titleArray  =  list(dict.fromkeys(title_list))
            urlArray = list(dict.fromkeys(url_list))
    array = []
    for item in range(len(titleArray)):
        dict = {}
        dict['title'] = titleArray[item]
        dict['url'] = urlArray[item]
        array.append(dict)
