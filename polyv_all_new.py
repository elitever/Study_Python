...
# 1.读取本地JSON数据,全局赋值
def reqeHeader():
    with open(saveRootDirPath + "/Cookie.txt") as f:
        header = {
            'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Safari/537.36',
            'Cookie': f.read()}
    return header
...
def getVid(url):
    if ('lib' in url):
        libvid = url[url.rindex('vid=') + 1:].replace('id=','').replace('.html','')
        return libvid
...
def dowloadMP4(url,homeurl):
    ...
    if response.status_code == 200:
        outputFp = open(saveRootDirPath + '/' + _title_catalog + '/' + _titleName + '.mp4', "wb+")
...
# 获得lib的url和title
def lib_Array_title_url():
    array = []
    # 解析本地html
    soup = BeautifulSoup(open(saveRootDirPath + '/home.html', encoding="utf-8"), 'html.parser')
    html = soup.find_all('div', class_='section-titlea')
    html_catalog = soup.find_all('div', class_='albums-post fn-fl')
    # 获取整个视频的title并且创造目录
    for her in html_catalog:
        mkdir(saveRootDirPath + '/' + her.img.get('title'))
    # 获取视频的标题和URL
    for href in html:
        eq_url = 'https://lib.yiihuu.com' + href.a.get('href')
    ...
    return {'array':array,'title_catalog':her.img.get('title')}
...
def lib_Dowload_zip():
    ...
    soup = BeautifulSoup(open(saveRootDirPath + '/home.html', encoding="utf-8"), 'html.parser')
    # 下载工程文件
    file_main = soup.find_all('div', class_='file_main')
    # 判断是否有工程文件
    if not file_main:
        print('没有工程文件')
    else:
        #   创建一个工程文件
        mkdir(saveRootDirPath + '/' + _title_catalog + '/' + '工程文件')
        for href in file_main:
            eq_url = 'https:' + href.a.get('href')
            title = href.p.get_text().strip()
            response = requests.get(eq_url, stream=True)
            zip = ''
            # 判断文件是否是zip后缀
            if (title.find('.zip') >= 0):
                zip = '.zip'
            ...
            # 写入本地文件
            with open(saveRootDirPath + '/' + _title_catalog + '/' + '工程文件' + '/' + title + zip, 'wb') as f:
                for chunk in response.iter_content(chunk_size=1024):
                    if chunk:
                        f.write(chunk)
                        print('正在下载工程文件：' + title)
    return 0
...
def DowLoadVide():
    # 解析本地html
    soup = BeautifulSoup(open(saveRootDirPath + '/home.html', encoding="utf-8"), 'html.parser')
    # print(saveRootDirPath + '/home.html')
    # 获取title来判断是否是学习可以或者是官网
    title = str(soup.head.title)

    if (title.find('翼狐设计学习库') >= 0):

        # 获取lib的tille
        libArray = lib_Array_title_url()
        ...
        # _title_catalog = soup.head.title.string.replace('视频教程_翼狐网','')
        # mkdir(saveRootDirPath + '/' + _title_catalog + '/' +'工程文件')

        # html = soup.find_all('div', class_='file_main')
        # for href in html:
        #     eq_url = 'https://www.yiihuu.com' +href.a.get('href')
        #     print(eq_url)
        #
        #     title = href.p.get_text().strip()
        #     response = requests.get(eq_url)
        #     outputFp = open(saveRootDirPath + '/' + _title_catalog +  '/' +'工程文件' +'/' + title + '.7z', "wb+")
        #     outputFp.write(response.content)
        #     print('下载完成')
        # print(response)
        # if response.status_code == 200:
...
def main(argc, argv,dictData) :
    ...
    # 10.convert to mp4（调用了FFmpeg，将合并好的视频内容放置到一个mp4容器中）
    # ffmpegConvertToMp4(cachePath + "/cache.flv", _title_catalog + '/' + title + ".mp4")
    ffmpegConvertToMp4(cachePath + "/cache.flv", saveRootDirPath + '/' + _title_catalog + '/' + title + ".mp4")
    # 11.删除ts文件
...
if __name__ == "__main__" :
    ...
    arrays = array['array']
    # 目录名称
    _title_catalog = array['title_catalog']
    for dictData in arrays:
       #  视频名称
    ...
    # for dictData in range(len(arrays)):
    #         if dictData ==10:
    #           # print(dictData)
    #           _titleName = arrays[dictData]['title']
    #           main(len(sys.argv), sys.argv,arrays[dictData])
