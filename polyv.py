...
def get_m3u8Key(m3u8,vid):
    ...
    m3u8keyurl = re.sub(r'://([^/]+)/', r'://\1/playsafe/v11/', m3u8keyurl, 1, re.I)
...
def decode_key(seed_const, key_enc) :
    ...
    cryptor = AES.new(aeskey.encode('utf-8'), AES.MODE_ECB)
...
def main(argc, argv) :
    ...
    # 10.convert to mp4（调用了FFmpeg，将合并好的视频内容放置到一个mp4容器中）
    ffmpegConvertToMp4(cachePath + "/cache.flv", saveRootDirPath + "/" + vinfo["title"] + ".mp4")
...
if __name__ == "__main__" :
    ...
    # _homeurl = 'https://lib.yiihuu.com/video/play.php?vid=208971'
